...
function ApproveDocument() {
  ...
  const fetchDocument = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5000/api/edms/${id}`, {
        method: 'GET',
        headers: {
  ...
  const handleApprovalSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/edms/${id}/approve`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleSignatureSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/edms/${id}/sign`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
