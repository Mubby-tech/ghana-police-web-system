...
function SystemSettings() {
  ...
  const fetchSettings = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/settings/system${queryParams}`, {
        method: 'GET',
        headers: {
  ...
  const handleSave = async (setting) => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/settings/system/${setting.key}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
